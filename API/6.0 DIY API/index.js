...
app.get("/random", (req, res) => {
  if (jokes.length === 0) {
    res.status(404).send({
      message: "There's no jokes available",
    });
  } else {
    const position = Math.floor(Math.random() * 100);
    res.json(jokes[position]);
  }
});
...
app.get("/jokes/:id", (req, res) => {
  if (jokes.length === 0) {
    res.status(404).send({
      message: "There's no joke available",
    });
  } else {
    ...
    if (foundJoke === undefined) {
      res.status(404).send({
        message: "Joke with the specified ID doesn't exist",
      });
    } else {
      res.json(foundJoke);
    }
...
app.get("/filter", (req, res) => {
  if (jokes.length === 0) {
    ...
    if (req.query.type) {
      const allJokes = jokes.filter((joke) => {
      ...
      res.status(404).send({ message: "Expected type as parameter" });
    }
  }
});
...
app.post("/jokes", (req, res) => {
  if (jokes.length === 0) {
    ...
    const data = req.body;
    if (data.hasOwnProperty("jokeText") && data.hasOwnProperty("jokeType")) {
      ...
      res
        .status(404)
        .send({ message: "Body does not contain the necessary properties" });
    }
  }
});
  ...
  if (jokes.length === 0) {
    ...
    if (data.hasOwnProperty("jokeText") || data.hasOwnProperty("jokeType")) {
      ...
      if (searchIndex === -1) {
        ...
        const newJoke = {
          id: jokes[searchIndex].id,
          jokeText: data.jokeText || jokes[searchIndex].jokeText,
          jokeType: data.jokeType || jokes[searchIndex].jokeType,
        };
        jokes[searchIndex] = newJoke;
        return res.json(jokes[searchIndex]);
      }
    } else {
      res
        .status(404)
